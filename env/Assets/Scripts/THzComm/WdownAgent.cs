...
public class WdownAgent : Agent
{
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        {
            mCoordi.cur_Wdown[i] = (1f / (2f * USERSIZE)) * Mathf.Clamp(actions.ContinuousActions[i], -1f, 1f) + (2f / (2f * USERSIZE));
        }
    ...
    bool IsContraint()
    {
        ...
        for (int i = 0; i < MAXUSER; ++i)
        {
            if (i < USERSIZE) sum += mCoordi.cur_Wdown[i];
            else mCoordi.cur_Wdown[i] = 0f;
        }
        if (sum > 1.0f)
        {
            return true;
