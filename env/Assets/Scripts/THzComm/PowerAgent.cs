...
public class PowerAgent : Agent
{
    ...
    {
        // 위치, 대역, 전력, 알파
        //sensor.AddObservation(USERSIZE);
        for (int i = 0; i < USERSIZE; ++i) // 15x6 = 90
        {
            ...
            //sensor.AddObservation(Vector2.Distance(mCoordi.cur_Pos, new Vector2(userPos.x, userPos.z)));
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        {
            mCoordi.cur_Pow[i] = (1f / (2f * USERSIZE)) * Mathf.Clamp(actions.ContinuousActions[i], -1f, 1f) + (2f / (2f * USERSIZE));
        }
    ...
    bool IsContraint()
    {
        ...
        for (int i = 0; i < MAXUSER; ++i)
        {
            if (i < USERSIZE) sum += mCoordi.cur_Pow[i];
            else mCoordi.cur_Pow[i] = 0f;
        }
        if (sum > 1.0f)
        {
            return true;
